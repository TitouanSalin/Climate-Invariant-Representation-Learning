#!/usr/bin/env python3
"""Run CMIP_variable_exp5_baseline_ClimaX notebook in batch mode and export artifacts."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Execute CMIP_variable_exp5_baseline_ClimaX notebook and export artifacts")
    parser.add_argument("--project-dir", default="/glade/u/home/tsalin/CMIP", help="Project directory containing notebooks/")
    parser.add_argument(
        "--notebook",
        default="notebooks/trainings/variable/CMIP_variable_exp5_baseline_ClimaX.ipynb",
    ...
    parser.add_argument(
        "--evaluation-dir",
        default="model_evaluation/Baseline_ClimaX",
        help="Output folder (relative to project dir) for history and quality dataframes",
    )
    parser.add_argument(
        "--executed-notebook",
        default="executed_notebooks/CMIP_variable_exp5_baseline_ClimaX.executed.ipynb",
...
if "climax_history_df" not in globals():
    raise ValueError("climax_history_df is missing. Ensure the training cell was executed.")
if "climax_quality_payload" not in globals():
    raise ValueError("climax_quality_payload is missing. Ensure the evaluation cell was executed.")
if "climax_latent_by_climate" not in globals() or "climax_latent_test_metadata_by_climate" not in globals():
    raise ValueError("ClimaX latent variables are missing. Ensure latent extraction cells were executed.")
...
climax_history_df.to_pickle(_history_path)
with open(_quality_path, "wb") as _qf:
    pickle.dump(climax_quality_payload, _qf)

climax_payload = _pack_latent_dict(climax_latent_by_climate, climax_latent_test_metadata_by_climate)
with open(_latent_path, "wb") as _f:
    pickle.dump(climax_payload, _f)
...
def main() -> int:
    ...
    print("[INFO] Exported ClimaX history, quality, and latent files successfully.")
    print("[DONE] CMIP_variable_exp5_baseline_ClimaX batch run completed.")
    return 0

#!/usr/bin/env python3
"""Run Experiment 3 notebook in batch mode and export artifacts.

This script executes the notebook:
CMIP/notebooks/trainings/variable/CMIP_distribution_and_AE_tas_exp3_res_only.ipynb

It then:
1) Saves all image/png notebook outputs into CMIP/plots_exp_3/
2) Saves latent representations into
   CMIP/latent_representations_exp_3/<prefix>latent_test_representations.pkl
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Execute Exp 3 notebook and export plots/latents")
    parser.add_argument(
        "--project-dir",
    ...
    parser.add_argument(
        "--notebook",
        default="notebooks/trainings/variable/CMIP_distribution_and_AE_tas_exp3_res_only.ipynb",
    ...
    parser.add_argument(
        "--latents-dir",
        default="latent_representations_exp_3",
        help="Output folder (relative to project dir) for latent files",
    )
    parser.add_argument(
        "--evaluation-dir",
        default="model_evaluation/exp3",
        help="Output folder (relative to project dir) for reconstruction dataframes",
    )
    parser.add_argument(
        "--executed-notebook",
        default="notebooks/CMIP_distribution_and_AE_tas_exp3_res_only.executed.ipynb",
    ...
    parser.add_argument(
        "--ae-setup-name",
        choices=["AEh", "AEhs2", "AEhs2s3", "AEall"],
        default=None,
        help="Override ae_setup_name",
    )
...
if "reconstruction_payload" not in globals():
    raise ValueError("reconstruction_payload is missing. Ensure the evaluation cell was executed.")
if "latent_test_by_climate" not in globals() or "latent_test_metadata_by_climate" not in globals():
    raise ValueError("Latent variables are missing. Ensure latent extraction cells were executed.")
...
with open(_recon_path, "wb") as _qf:
    pickle.dump(reconstruction_payload, _qf)

payload = _pack_latent_dict(latent_test_by_climate, latent_test_metadata_by_climate)
...
def main() -> int:
    ...
    overrides = {
        ...
        for k, v in {
            ...
            "ae_setup_name": args.ae_setup_name,
    ...
    print("[DONE] Exp 3 batch run completed.")
    return 0

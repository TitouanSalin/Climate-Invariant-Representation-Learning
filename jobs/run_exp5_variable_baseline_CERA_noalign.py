#!/usr/bin/env python3
"""Run CMIP_variable_exp5_baseline_CERA_noalign notebook in batch mode and export artifacts."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Execute CMIP_variable_exp5_baseline_CERA_noalign notebook and export artifacts")
    parser.add_argument("--project-dir", default="/glade/u/home/tsalin/CMIP", help="Project directory containing notebooks/")
    parser.add_argument(
        "--notebook",
        default="notebooks/trainings/variable/CMIP_variable_exp5_baseline_CERA_noalign.ipynb",
    ...
    parser.add_argument(
        "--evaluation-dir",
        default="model_evaluation/Baseline_CERA_noalign",
        help="Output folder (relative to project dir) for history and quality dataframes",
    )
    parser.add_argument(
        "--executed-notebook",
        default="notebooks/CMIP_variable_exp5_baseline_CERA_noalign.executed.ipynb",
...
if "no_align_history_df" not in globals():
    raise ValueError("no_align_history_df is missing. Ensure the training cell was executed.")
if "baseline_no_align_quality_payload" not in globals():
    raise ValueError("baseline_no_align_quality_payload is missing. Ensure the evaluation cell was executed.")
if "no_align_latent_by_climate" not in globals() or "no_align_latent_test_metadata_by_climate" not in globals():
    raise ValueError("Baseline no-align latent variables are missing. Ensure latent extraction cells were executed.")
...
no_align_history_df.to_pickle(_history_path)
with open(_quality_path, "wb") as _qf:
    pickle.dump(baseline_no_align_quality_payload, _qf)

payload = _pack_latent_dict(no_align_latent_by_climate, no_align_latent_test_metadata_by_climate)
...
def main() -> int:
    ...
    print("[INFO] Exported baseline no-align history, quality, and latent files successfully.")
    print("[DONE] CMIP_variable_exp5_baseline_CERA_noalign batch run completed.")
    return 0
